...
export async function GET(request: Request) {
    ...
    return {
      ...
      lastPaymentAt:
        b.payments.length > 0
          ? b.payments.reduce(
              (latest, p) =>
                new Date(p.paidAt) > new Date(latest) ? p.paidAt : latest,
              b.payments[0].paidAt,
            )
          : null,
    };
  ...
  const tasaCobranza =
    facturado === 0 ? 0 : Math.round((cobrado / facturado) * 100);
  ...
  const customerCollected = new Map<
    string,
    { name: string; email: string; paid: number; count: number }
  >();
  for (const i of items) {
