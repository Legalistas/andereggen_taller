...
export async function GET(request: Request) {
    ...
    const rows = customers.map((c) => {
        ...
        const conversionRate = budgetCount === 0 ? 0 : Math.round((acceptedCount / budgetCount) * 100);
        ...
        const lastLead = c.leads.reduce<Date | null>(
            (latest, l) => (!latest || new Date(l.updatedAt) > latest ? new Date(l.updatedAt) : latest),
            null,
        );
        const lastBudget = myBudgets.reduce<Date | null>(
            (latest, b) => (!latest || new Date(b.createdAt) > latest ? new Date(b.createdAt) : latest),
            null,
        );
        ...
        if (pendingBalance > 0) seg = "moroso"; // moroso tiene prioridad sobre los demás
        ...
        return {
            ...
            firstContactAt: lastLead && c.leads.length > 0
                ? c.leads.reduce((earliest, l) => (new Date(l.createdAt) < new Date(earliest) ? l.createdAt : earliest), c.leads[0].createdAt).toISOString()
                : null,
            segment: seg,
        };
    });
    ...
    // ── Nuevos por mes ──────────────────────────────────────────
    const monthMap = new Map<string, { month: string; label: string; newCount: number }>();
    ...
    while (cursor <= endCursor) {
        ...
        monthMap.set(key, { month: key, label: fmtLabel.format(cursor), newCount: 0 });
    ...
    const newByMonth = Array.from(monthMap.values()).sort((a, b) => a.month.localeCompare(b.month));
