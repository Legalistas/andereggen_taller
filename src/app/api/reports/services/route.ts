...
export async function GET(request: Request) {
    ...
    const scope = (url.searchParams.get("scope") ?? "accepted") as "accepted" | "all";

    // Filtro de budget según scope
    const budgetStatuses: BudgetStatus[] =
        scope === "accepted" ? ["accepted"] : ["draft", "sent", "accepted", "rejected", "expired"];
    ...
    const [concepts, partsAggregate, budgetIds] = await Promise.all([
        prisma.budgetConcept.findMany({
            ...
            select: {
                ...
                units: true,
                unitValue: true,
                budget: { select: { id: true, [dateKey]: true, createdAt: true, acceptedAt: true } },
            },
        }),
    ...
    const byCategory = Array.from(byCategoryMap.values())
        .map((e) => ({
            ...
            avgUnitValue: e.unitValueCount === 0 ? null : e.unitValueSum / e.unitValueCount,
        }))
    ...
    const monthMap = new Map<
        string,
        { month: string; label: string; DESCRIPTIVO: number; UNIDADES: number; FIJO: number; partsAmount: number }
    >();
    ...
    const monthlyByType = Array.from(monthMap.values()).sort((a, b) => a.month.localeCompare(b.month));

    // ── KPIs ───────────────────────────────────────────────────
    const totalPartsAmount = Number(partsAggregate._sum.subtotal ?? 0);
    const grandTotal = totalLaborAmount + totalPartsAmount;
    const laborShare = grandTotal === 0 ? 0 : Math.round((totalLaborAmount / grandTotal) * 100);
    ...
    return NextResponse.json({
        ...
        byType,
        monthlyByType,
        topCategories: byCategory.filter((c) => c.type !== "DESCRIPTIVO").slice(0, 10),
    });
}
