...
import type { BudgetStatus, LeadStatus } from "../../../../generated/prisma/client";
...
const ACTIVE_LEAD_STATUSES: LeadStatus[] = ["solicitud", "control", "enviado", "refuerzo"];
...
export async function GET(request: Request) {
    ...
    const [
        ...
        prisma.$queryRaw<Array<{ month: string; emitted: bigint; accepted: bigint; accepted_amount: string }>>`
            SELECT
    ...
    // KPIs
    const totalBudgets = Object.values(budgetStatus).reduce((a, b) => a + b.count, 0);
    ...
    const conversionRate = totalBudgets === 0 ? 0 : Math.round((acceptedCount / totalBudgets) * 100);
    const avgTicket = acceptedCount === 0 ? 0 : Math.round(invoiced / acceptedCount);
...
function buildMonthlyBuckets(
    from: Date,
    to: Date,
    rows: Array<{ month: string; emitted: bigint; accepted: bigint; accepted_amount: string }>,
): MonthBucket[] {
