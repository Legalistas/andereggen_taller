...
export default function SourcesTab() {
  ...
  const submit = async () => {
    ...
    if (!editing && !form.key.trim()) {
      setError("La key es obligatoria");
      return;
    }
    setBusy(true);
    setError(null);
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {editing
                ? "La key no se puede cambiar; sólo label, orden y estado."
          ...
          <div className="grid gap-3 py-2">
            ...
            <div className="flex items-center justify-between gap-2 pt-1">
              <Label htmlFor="srcActive" className="cursor-pointer">
                Activa
              </Label>
              <Switch
                id="srcActive"
        ...
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {deleteTarget && (
                <>
                  ¿Eliminar <b>{deleteTarget.label}</b> (
                  <code className="text-xs">{deleteTarget.key}</code>)? Los
                  leads existentes con esta fuente conservan el valor pero no
                  aparecerá más en nuevos leads.
                </>
              )}
            </DialogDescription>
          </DialogHeader>
