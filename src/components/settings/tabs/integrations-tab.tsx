...
export default function IntegrationsTab({ settings, onSave }: Props) {
  ...
  const submit = async () => {
    ...
    setMsg(
      res.ok
        ? { ok: true, text: "Integraciones guardadas." }
      ...
      <Card className="p-6">
        ...
        <div className="flex items-start gap-2 rounded-md border border-border bg-muted/40 px-3 py-2 text-sm mb-4">
          ...
          <span className="text-xs">
            Estas integraciones están listas para configurar pero los conectores
            aún no ejecutan llamadas reales. Cuando activemos cada una, las
            credenciales de acá se van a usar automáticamente.
          </span>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="grid gap-1.5">
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, whatsappApiKey: e.target.value }))
                }
              />
            </div>
          </div>
        ...
        >
          <div className="grid gap-1.5">
            ...
            <p className="text-xs text-muted-foreground">
              Generalo en la consola de MercadoPago → Credenciales de
              producción.
            </p>
          </div>
        </IntegrationCard>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="grid gap-1.5">
              ...
              <Input
                ...
                value={form.afipCuit}
                onChange={(e) =>
                  setForm((f) => ({ ...f, afipCuit: e.target.value }))
                }
            ...
            <div className="grid gap-1.5 md:col-span-2">
              ...
              <Textarea
                id="afipCert"
                rows={2}
                value={form.afipCertNote}
                onChange={(e) =>
                  setForm((f) => ({ ...f, afipCertNote: e.target.value }))
                }
                placeholder="Ubicación del .pfx, fecha de vencimiento, contraseña..."
              />
              <p className="text-xs text-muted-foreground">
                El certificado .pfx real NO se sube desde acá; se carga por ENV
