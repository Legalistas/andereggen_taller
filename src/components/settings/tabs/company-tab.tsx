...
export default function CompanyTab({ settings, onSave }: Props) {
    ...
    const submit = async () => {
        ...
        setBusy(false)
        setMsg(res.ok ? { ok: true, text: "Datos de la empresa guardados." } : { ok: false, text: res.error ?? "Error" })
    }
        ...
        <Card className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="grid gap-1.5 md:col-span-2">
                    ...
                    <Textarea id="companyAddress" rows={2} value={form.companyAddress} onChange={(e) => setForm((f) => ({ ...f, companyAddress: e.target.value }))} />
                </div>
                <div className="grid gap-1.5">
                    <Label htmlFor="companyCuit">CUIT</Label>
                    <Input id="companyCuit" value={form.companyCuit} onChange={(e) => setForm((f) => ({ ...f, companyCuit: e.target.value }))} placeholder="30-12345678-9" />
                </div>
                <div className="grid gap-1.5">
                    <Label htmlFor="companyPhone">Teléfono</Label>
                    <Input id="companyPhone" value={form.companyPhone} onChange={(e) => setForm((f) => ({ ...f, companyPhone: e.target.value }))} placeholder="(03492) 155-90753" />
                </div>
                <div className="grid gap-1.5">
                    <Label htmlFor="companyEmail">Email</Label>
                    <Input id="companyEmail" type="email" value={form.companyEmail} onChange={(e) => setForm((f) => ({ ...f, companyEmail: e.target.value }))} placeholder="contacto@empresa.com" />
                </div>
                <div className="grid gap-1.5">
                    <Label htmlFor="companyWebsite">Sitio web</Label>
                    <Input id="companyWebsite" value={form.companyWebsite} onChange={(e) => setForm((f) => ({ ...f, companyWebsite: e.target.value }))} placeholder="https://empresa.com" />
                </div>
                <div className="grid gap-1.5 md:col-span-2">
                    <Label htmlFor="companyLogoUrl">URL del logo</Label>
                    <Input id="companyLogoUrl" value={form.companyLogoUrl} onChange={(e) => setForm((f) => ({ ...f, companyLogoUrl: e.target.value }))} placeholder="https://…/logo.png" />
                    <p className="text-xs text-muted-foreground">Se usa en la cabecera de los PDFs de presupuesto.</p>
                </div>
            </div>
