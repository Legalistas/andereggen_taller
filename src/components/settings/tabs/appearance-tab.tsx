        ...
        <Card className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="grid gap-1.5">
                    ...
                    <Select value={form.locale} onValueChange={(v) => setForm((f) => ({ ...f, locale: v }))}>
                        ...
                        <SelectContent>
                            {LOCALES.map((l) => <SelectItem key={l.key} value={l.key}>{l.label}</SelectItem>)}
                        </SelectContent>
                    </Select>
                </div>
                <div className="grid gap-1.5">
                    ...
                    <Select value={form.currency} onValueChange={(v) => setForm((f) => ({ ...f, currency: v }))}>
                        <SelectTrigger><SelectValue /></SelectTrigger>
                        <SelectContent>
                            {CURRENCIES.map((c) => <SelectItem key={c.key} value={c.key}>{c.label}</SelectItem>)}
                        </SelectContent>
                    </Select>
                </div>
                <div className="grid gap-1.5">
            ...
            <div className="mt-4 rounded-md border bg-muted/30 p-3 text-sm">
                ...
                <div className="text-xs text-muted-foreground mt-1 space-y-0.5">
                    <div>Moneda: <b>{new Intl.NumberFormat(form.locale, { style: "currency", currency: form.currency }).format(123456.78)}</b></div>
                    <div>Fecha/hora: <b>{new Intl.DateTimeFormat(form.locale, { dateStyle: "medium", timeStyle: "short", timeZone: form.timezone }).format(new Date())}</b></div>
                </div>
            </div>
