...
import CustomerSelector, { type Customer } from "@/components/crm/customer-selector"
...
const ARS_PRECISE = new Intl.NumberFormat("es-AR", { style: "currency", currency: "ARS", minimumFractionDigits: 2 })
const INT = new Intl.NumberFormat("es-AR", { maximumFractionDigits: 2 })

type StatusStyle = { label: string; color: string; icon: React.ComponentType<{ className?: string }> }
const STATUS_CONFIG: Record<BudgetStatus, StatusStyle> = {
    draft: { label: "Borrador", color: "bg-slate-500/10 text-slate-700 border-slate-200", icon: FileText },
    sent: { label: "Enviado", color: "bg-cyan-500/10 text-cyan-700 border-cyan-200", icon: Send },
    accepted: { label: "Aceptado", color: "bg-green-500/10 text-green-700 border-green-200", icon: Trophy },
    rejected: { label: "Rechazado", color: "bg-red-500/10 text-red-700 border-red-200", icon: XCircle },
    expired: { label: "Vencido", color: "bg-gray-500/10 text-gray-700 border-gray-200", icon: Calendar },
}
...
export default function QuotesSection() {
    ...
    const fetchQuotes = useCallback(async (signal?: AbortSignal) => {
        ...
        try {
            ...
            if ((e as Error).name !== "AbortError") {
                setLoadError(e instanceof Error ? e.message : "Error al cargar cotizaciones")
            }
        } finally {
            setLoading(false)
        }
    ...
    const stats = useMemo(() => {
        ...
        const accepted_amount = quotes
            ...
            .reduce((acc, q) => acc + Number(q.grandTotal), 0)
        return { total, sent, accepted, rejected, conversion, pipeline, accepted_amount }
    }, [quotes])
    ...
    const resetNewForm = () => {
        setNewCustomer(null)
        setNewVehicleMode("")
        setNewBrand(""); setNewModel(""); setNewYear("")
        setNewPlate(""); setNewInsurance(""); setNewThirdPartyInsurance("")
        setNewError(null)
    }

    const handleContinueToQuote = async () => {
        setNewError(null)
        if (!newCustomer) { setNewError("Seleccioná un cliente."); return }
        ...
        if (newVehicleMode === "new") {
            ...
            payload.newVehicle = {
                brand: newBrand, model: newModel, year: newYear, domain: newPlate,
                secure: newInsurance, thirdPartySecure: newThirdPartyInsurance,
            }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                <div>
                    <Breadcrumbs items={[{ label: "CRM" }, { label: "Cotizaciones" }]} />
                    <h1 className="text-3xl font-bold">Cotizaciones</h1>
                    <p className="text-sm text-muted-foreground mt-1">
                        Vista global de todos los presupuestos emitidos. Creá una cotización aparte sin entrar al lead.
                    </p>
                </div>
                <Button className="gap-2" onClick={() => { resetNewForm(); setNewOpen(true) }}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                <StatCard label="Emitidas" value={String(stats.total)} icon={FileText} color="blue" />
                <StatCard label="Enviadas" value={String(stats.sent)} icon={Send} color="cyan" />
                <StatCard label="Aceptadas" value={String(stats.accepted)} icon={Trophy} color="green" />
                <StatCard label="Tasa conversión" value={`${stats.conversion}%`} icon={TrendingUp} color="purple" />
                <StatCard label="Pipeline $" value={ARS.format(stats.pipeline)} icon={Calendar} color="orange" />
            </div>
            ...
            <Card className="p-4 space-y-4">
                <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    ...
                    />
                    <Button variant="ghost" size="sm" onClick={() => fetchQuotes()} className="gap-2">
                            ...
                            return (
                                <TableRow key={q.id}>
                                    ...
                                    <TableCell>
                                        ...
                                        <div className="text-xs text-muted-foreground font-mono">{q.vehicleDomain}</div>
                                    </TableCell>
                                            ...
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end">
                                                    {allowed.includes("sent") && (
                                                        <DropdownMenuItem className="gap-2" onClick={() => changeStatus(q.id, "sent")}>
                                                            <Send className="h-4 w-4 text-cyan-600" /> Marcar enviado
                                                        </DropdownMenuItem>
                                                    )}
                                                    {allowed.includes("accepted") && (
                                                        <DropdownMenuItem className="gap-2" onClick={() => changeStatus(q.id, "accepted")}>
                                                            <CheckCircle2 className="h-4 w-4 text-green-600" /> Marcar aceptado
                                                        </DropdownMenuItem>
                                                    )}
                                                    {allowed.includes("rejected") && (
                                                        <DropdownMenuItem className="gap-2 text-destructive" onClick={() => changeStatus(q.id, "rejected")}>
                                                            <XCircle className="h-4 w-4" /> Marcar rechazado
                                                        </DropdownMenuItem>
                                                    )}
                                                    {allowed.includes("expired") && (
                                                        <DropdownMenuItem className="gap-2" onClick={() => changeStatus(q.id, "expired")}>
                                                            <Calendar className="h-4 w-4 text-gray-600" /> Marcar vencido
                                                        </DropdownMenuItem>
                                                    )}
                                                    <DropdownMenuItem className="gap-2" onClick={() => duplicateQuote(q.id)}>
                                                        <Copy className="h-4 w-4" /> Duplicar
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem asChild>
                                                        <Link href="/crm/leads" className="gap-2 flex items-center w-full">
            ...
            <Dialog open={newOpen} onOpenChange={(v) => { setNewOpen(v); if (!v) resetNewForm() }}>
                <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Elegí cliente y vehículo. Al continuar se crea el lead (en "control") y se abre el editor de presupuesto.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="grid gap-6 py-4">
                        ...
                        {newCustomer && (
                            <div className="space-y-3">
                                ...
                                <Select value={newVehicleMode} onValueChange={setNewVehicleMode}>
                                    <SelectTrigger>
                        ...
                        {newError && (
                            <div className="flex items-start gap-2 rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm text-destructive">
                                <AlertCircle className="h-4 w-4 mt-0.5 shrink-0" /> <span>{newError}</span>
                            </div>
                        )}
                    </div>
                    <DialogFooter>
                        <Button variant="outline" onClick={() => setNewOpen(false)} disabled={newBusy}>Cancelar</Button>
                        <Button onClick={handleContinueToQuote} disabled={newBusy} className="gap-2">
                            {newBusy ? <><Loader2 className="h-4 w-4 animate-spin" /> Creando…</> : <>Continuar al presupuesto <Check className="h-4 w-4" /></>}
                        </Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <BudgetModal
                ...
                onOpenChange={(v) => { if (!v) setBudgetLeadId(null) }}
                onSaved={() => { setBudgetLeadId(null); fetchQuotes() }}
            />
            ...
            <Dialog open={!!detailId} onOpenChange={(v) => { if (!v) setDetailId(null) }}>
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>Detalle read-only del presupuesto.</DialogDescription>
                    </DialogHeader>
                        ...
                        <div className="space-y-5">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                <div className="space-y-1">
                                    ...
                                    <div className="font-medium flex items-center gap-1"><User className="h-3 w-3" /> {detail.customerName}</div>
                                    <div className="text-xs flex items-center gap-1 text-muted-foreground"><Mail className="h-3 w-3" /> {detail.customerEmail}</div>
                                    <div className="text-xs flex items-center gap-1 text-muted-foreground"><Phone className="h-3 w-3" /> {detail.customerPhone}</div>
                                    {detail.customerDni && <div className="text-xs text-muted-foreground">DNI: {detail.customerDni}</div>}
                                </div>
                                <div className="space-y-1">
                                    <div className="text-xs uppercase tracking-wide text-muted-foreground">Vehículo</div>
                                    <div className="font-medium flex items-center gap-1">
                                        <Car className="h-3 w-3" /> {detail.vehicleBrand} {detail.vehicleModel} {detail.vehicleYear}
                                    </div>
                                    <div className="text-xs text-muted-foreground font-mono">{detail.vehicleDomain}</div>
                                    {detail.vehicleInsurance && <div className="text-xs text-muted-foreground">Seguro: {detail.vehicleInsurance}</div>}
                                </div>
                            </div>

                            <div className="flex flex-wrap items-center gap-3">
                                <Badge variant="outline" className={`gap-1 ${STATUS_CONFIG[detail.status].color}`}>
                                ...
                                {detail.acceptedAt && (
                                    <span className="text-xs text-green-600">
                                        Aceptada: {new Date(detail.acceptedAt).toLocaleString("es-AR")}
                                    </span>
                                )}
                                {detail.rejectedAt && (
                                    <span className="text-xs text-destructive">
                                        Rechazada: {new Date(detail.rejectedAt).toLocaleString("es-AR")}
                                    </span>
                                )}
                            </div>
                            ...
                            <div className="space-y-2">
                                ...
                                {detail.concepts.length === 0 ? (
                                    <div className="text-xs italic text-muted-foreground">Sin conceptos.</div>
                                ) : (
                                    <div className="rounded-md border divide-y">
                                        {detail.concepts.map((c) => (
                                            <div key={c.id} className="p-3 flex items-center justify-between gap-3">
                                                <div className="min-w-0">
                                                    <div className="font-medium text-sm">{c.category.replaceAll("_", " ")}</div>
                                                    <div className="text-xs text-muted-foreground">
                                                        {c.type === "DESCRIPTIVO" && (c.subdetails.length > 0
                                                            ? `${c.subdetails.length} subdetalles`
                                                            : "sin subdetalles")}
                                                        {c.type === "UNIDADES" && `${INT.format(Number(c.units))} × ${ARS_PRECISE.format(Number(c.unitValue))}`}
                                                        {c.type === "FIJO" && (c.fixedDescription ?? "Importe fijo")}
                                                    </div>
                                                </div>
                                                {c.type !== "DESCRIPTIVO" && (
                            ...
                            <div className="space-y-2">
                                ...
                                {detail.parts.length === 0 ? (
                                    <div className="text-xs italic text-muted-foreground">Sin repuestos.</div>
                                ) : (
                                    <Table>
                                        <TableHeader>
                                            <TableRow>
                                        ...
                                        <TableBody>
                                            {detail.parts.map((p) => (
                                                <TableRow key={p.id}>
                                                    <TableCell className="text-sm">{INT.format(Number(p.quantity))}</TableCell>
                                                    <TableCell className="text-sm">{p.description}</TableCell>
                                                    <TableCell className="text-right text-sm">{ARS_PRECISE.format(Number(p.unitPrice))}</TableCell>
                                                    <TableCell className="text-right text-sm font-medium">{ARS_PRECISE.format(Number(p.subtotal))}</TableCell>
                                                </TableRow>
                                            ))}
                                        </TableBody>
                                    </Table>
                            ...
                            <Card className="p-4">
                                <dl className="space-y-1 text-sm max-w-sm ml-auto">
                                    ...
                                    <div className="flex justify-between">
                                        <dt className="text-muted-foreground">IVA {Number(detail.ivaRate)}%</dt>
                                    ...
                                    <div className="border-t pt-2 mt-2 flex justify-between items-baseline">
                                        ...
                                        <dd className="font-bold text-lg text-[#003b73]">{ARS_PRECISE.format(Number(detail.grandTotal))}</dd>
                                    </div>
                                </dl>
                            </Card>
                            ...
                            <div className="text-xs text-muted-foreground border-t pt-3 space-y-0.5">
                                <div>Validez: {detail.validityDays} días · Entrega estimada: {detail.deliveryDays} días · Pago: {detail.paymentCondition}</div>
                                {detail.observations && <div className="italic mt-1">{detail.observations}</div>}
                            </div>

                            {statusError && (
                                <div className="flex items-start gap-2 rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm text-destructive">
                                    <AlertCircle className="h-4 w-4 mt-0.5 shrink-0" /> <span>{statusError}</span>
                                </div>
                            )}
                        </div>
                    )}
                        ...
                        {detail && ALLOWED_TRANSITIONS[detail.status].length > 0 && (
                            <>
                                {ALLOWED_TRANSITIONS[detail.status].includes("sent") && (
                                    <Button onClick={() => changeStatus(detail.id, "sent")} disabled={statusBusy} className="gap-2">
                                        <Send className="h-4 w-4" /> Marcar enviado
                                    </Button>
                                )}
                                {ALLOWED_TRANSITIONS[detail.status].includes("accepted") && (
                                    <Button onClick={() => changeStatus(detail.id, "accepted")} disabled={statusBusy} className="gap-2 bg-green-600 hover:bg-green-700">
                                        <CheckCircle2 className="h-4 w-4" /> Aceptar
                                    </Button>
                                )}
                                {ALLOWED_TRANSITIONS[detail.status].includes("rejected") && (
                                    <Button onClick={() => changeStatus(detail.id, "rejected")} disabled={statusBusy} variant="destructive" className="gap-2">
