...
const roleConfig: Record<string, RoleStyle> = {
  ...
  admin_taller: {
    ...
    color: "bg-[#003b73]/10 text-[#003b73] border-[#003b73]/20",
    icon: Shield,
  },
  contable: {
    label: "Contable",
...
export default function UsersSection({ currentUserId }: Props) {
  ...
  const isSelf = (u: UserDataType | null) =>
  ...
  const handleSendTestEmail = async () => {
    ...
    try {
      ...
      setEmailResult({
        ok: false,
        msg: e instanceof Error ? e.message : "Error al enviar",
      });
    } finally {
  ...
  const stats = {
    ...
    admins: users.filter(
      (u) =>
        u.role?.name === "super_admin" || u.role?.name === "admin_taller",
    ).length,
    externos: users.filter(
      (u) =>
        u.role?.name &&
        ["cliente", "inspector", "productor_seguros", "compania_seguros"].includes(
          u.role.name,
        ),
    ).length,
  };
        ...
        >
          ...
          <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Complete los datos del usuario y asigne un rol en el sistema
              </DialogDescription>
            </DialogHeader>
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                >
                  ...
                  <SelectContent>
                    {roles.map((r) => {
                      const style = roleConfig[r.name] ?? DEFAULT_ROLE_STYLE;
        ...
        <Table>
          ...
          <TableBody>
            ...
            {paginatedUsers.map((user) => {
              const roleStyle =
                (user.role && roleConfig[user.role.name]) ?? DEFAULT_ROLE_STYLE;
              ...
              return (
                <TableRow key={user.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant="outline"
                      className={`gap-1 ${roleStyle.color}`}
                    >
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem
                          className="gap-2"
                          onClick={() =>
                            openEmailDialog(user.email ?? "", user.name ?? "")
                          }
                        >
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm pt-2 border-t">
                <div>
                  ...
                  {(() => {
                    const rs =
                      (profileUser.role && roleConfig[profileUser.role.name]) ??
                      DEFAULT_ROLE_STYLE;
                    const Icon = rs.icon;
                    return (
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Actualizá nombre y email del usuario.
            </DialogDescription>
          </DialogHeader>
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {roleUser &&
                `Asignar un nuevo rol a ${roleUser.name ?? roleUser.email}.`}
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-2">
            <div className="grid gap-1.5">
              ...
              <Select
                ...
                onValueChange={setRoleSelectValue}
                disabled={actionBusy}
              >
                <SelectTrigger>
                  <SelectValue placeholder="Seleccionar rol" />
          ...
          <DialogFooter>
            ...
            <Button
              onClick={submitRole}
              disabled={actionBusy || !roleSelectValue}
              className="gap-2"
            >
              {actionBusy ? (
                <>
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Esta acción es permanente. Si el usuario tiene leads o
              presupuestos asociados, se recomienda <b>bloquearlo</b> en lugar
              de eliminarlo.
            </DialogDescription>
          </DialogHeader>
          {deleteUser && (
      ...
      >
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Dispara la plantilla <code className="text-xs">SampleEmail</code>{" "}
              vía SMTP. Los usuarios actuales son mock — editá el destinatario
              si querés que te llegue a vos.
            </DialogDescription>
          </DialogHeader>
