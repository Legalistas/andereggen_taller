  ...
  return (
    <EmailLayout preview={`Recordatorio — presupuesto #${budgetNumber}`}>
      ...
      <Text className="text-sm text-slate-700 leading-relaxed m-0 mb-4">
        ...
        <strong>#{budgetNumber}</strong> por <strong>{grandTotal}</strong> para
        tu <strong>{vehicle}</strong>. Queríamos saber si tuviste chance de
        revisarlo y si tenés alguna consulta.
      </Text>

      <Text className="text-sm text-slate-700 leading-relaxed m-0 mb-4">
        Si necesitás ajustar algún ítem o tenés preguntas, respondé este correo
        y te contestamos en el día.
      </Text>
      ...
      <Text className="text-xs text-slate-500 leading-relaxed m-0">
        Si ya tomaste una decisión o elegiste otro taller, avisanos así cerramos
        el tema desde nuestro lado. ¡Gracias!
      </Text>
    </EmailLayout>
  );
}
